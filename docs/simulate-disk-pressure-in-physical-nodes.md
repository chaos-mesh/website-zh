...
| size | s | 指定读取多少数据，不论 process-num 的值为多少，除非发生错误或者指定路径文件的大小小于每个 [dd](https://man7.org/linux/man-pages/man1/dd.1.html) 进程所读取的值，程序都只会读取 size 参数指定的数量。 | string 类型，默认为""，合法形式为一个整数加一个单位，如：1M、512kB。支持的单位有 c=1, w=2, b=512, kB=1000, K=1024, MB=1000\*1000,M=1024\*1024, , GB=1000\*1000\*1000, G=1024\*1024\*1024 BYTE等， size 不能为"" |
...
| process-num | n | 指定使用多少个并发运行的 [dd](https://man7.org/linux/man-pages/man1/dd.1.html) 进程执行程序。 | uint8 类型，默认值为 1，范围为1-255 |
| size | s | 指定写入多少数据，不论 process-num 的值为多少，除非发生错误，程序都只会写入 size 参数指定的数量。 | string 类型，默认为""，合法形式为一个整数加一个单位，如：1M、512kB。支持的单位有 c=1, w=2, b=512, kB=1000, K=1024, MB=1000\*1000,M=1024\*1024, , GB=1000\*1000\*1000, G=1024\*1024\*1024 BYTE等， size 不能为"" |
...
| size      | s        | 指定写入多少数据，不论 process-num 的值为多少，除非发生错误，程序都只会写入 size 参数指定的数量。 | string 类型，默认为""，合法形式为一个整数加一个单位，如：1M、512kB。支持的单位有 c =1, w =2, b =512, kB =1000, K =1024, MB =1000\*1000,M =1024\*1024, , GB =1000\*1000\*1000, G =1024\*1024\*1024 BYTE 等，size 不能和 percent 都为"" |
